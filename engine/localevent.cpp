...
bool LocalEvent::EmulateMouseAction(KeySym key)
{
	...
	if(keystate[emulate_mouse_down])
	{
	    mouse_cu.y += emulate_mouse_step;
	    SetModes(MOUSE_MOTION);
	}
	...
	if(keystate[emulate_mouse_left])
	{
	...
	if(keystate[emulate_mouse_right])
	{
	    mouse_cu.x += emulate_mouse_step;
	    SetModes(MOUSE_MOTION);
	}
	...
	// if(keystate[emulate_press_left])
	{
	...
	// if(keystate[emulate_press_right])
	{
	    if(modes & KEY_PRESSED)
	    {
		mouse_pr = mouse_cu;
